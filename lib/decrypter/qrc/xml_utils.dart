...
class XmlUtils {
      ...
      if (result[i] == '<') {
        ...
        if (i > 0 &&
          ...
          if (part.contains('=') &&
            ...
            // 检查等号和左括号之间是否有空格
            final part1 = result.substring(
                leftBracketPos, leftBracketPos + part.indexOf('='));
            if (!part1.trim().contains(' ')) {
              // 删除无效标签，重置索引
              result =
                  result.substring(0, leftBracketPos) + result.substring(i + 1);
              i = 0;
              continue;
            }
          }
  ...
  /// [resDict] 结果集
  static void recursionFindElement(XmlNode xmlNode,
      Map<String, String> mappingDict, Map<String, XmlNode> resDict) {
    // 检查节点是否为元素节点并且有名称
