    ...
    try {
      ...
      if (!ignoreSyllable) {
        ...
        if (checkHeader200(trackGet)) {
          final String? lyrics = trackGet?['message']?['body']?['richsync']?['richsync_body'];
          if (lyrics != null && lyrics.isNotEmpty) {
            final List<dynamic> list = json.decode(lyrics);
            final richSyncLines = list.map((e) => RichSyncedLine.fromJson(e)).toList();
      ...
      if (checkHeader200(trackGetSubs)) {
        final List<dynamic>? list = trackGetSubs?['message']?['body']?['subtitle_list'];
      ...
      if (checkHeader200(trackGetLyrics)) {
        final String? lyrics = trackGetLyrics?['message']?['body']?['lyrics']?['lyrics_body'];
        if (lyrics != null && lyrics.isNotEmpty) {
          var lines = lyrics.trim().split('\n').map((line) => LineInfo.fromText(line)).toList();
          return LyricsData()
            ..file = (FileInfo()
              ..type = LyricsTypes.musixmatch
